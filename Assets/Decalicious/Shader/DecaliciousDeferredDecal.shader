...
Shader "Decalicious/Deferred Decal"
{
	...
	SubShader
	{
		...
		Pass
		{
			...
			void frag(v2f i, out float4 outAlbedo : COLOR0, out float4 outEmission : COLOR1)
			{
		...
		Pass
		{
			...
			void frag(v2f i, out float4 outSpecSmoothness : COLOR0, out float4 outNormal : COLOR1)
			{
				DEFERRED_FRAG_HEADER
				...
				float4 newSpecSmoothness = float4(tex2D(_SpecularTex, texUV).rgb * _SpecularMultiplier.rgb,
												  tex2D(_SmoothnessTex, texUV).r * _SmoothnessMultiplier);

				// Write, interpolate between old and new values
				outSpecSmoothness = lerp(specSmoothness, newSpecSmoothness, mask * _SpecularMultiplier.a);
			}
			ENDCG
		}
	}
